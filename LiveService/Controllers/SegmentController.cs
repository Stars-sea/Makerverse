...
public class SegmentController(
    ...
    public async Task<ActionResult<List<string>>> GetSegments(string liveId, CancellationToken ct) {
        ...
        await foreach (string segment in persistentService.ListSegmentsAsync(liveId, ct)) segments.Add(segment);
        return segments;
    }
    ...
    public async Task GetPlaylist(string liveId, CancellationToken ct) {
        ...
        ErrorOr<ObjectStat> statRet = await persistentService.GetPlaylistStatAsync(liveId, ct);
        if (statRet.IsError) {
            Response.StatusCode = StatusCodes.Status404NotFound;
        ...
        ErrorOr<ObjectStat> ret = await persistentService.GetPlaylistAsync(liveId, Response.Body, ct);
        if (ret.IsError) {
            Response.StatusCode = StatusCodes.Status404NotFound;
    ...
    public async Task GetSegment(string liveId, int index, CancellationToken ct) {
        ...
        ErrorOr<ObjectStat> statRet = await persistentService.GetSegmentStatAsync(liveId, index, ct);
        if (statRet.IsError) {
            Response.StatusCode = StatusCodes.Status404NotFound;
            await Response.WriteAsJsonAsync(statRet.Errors, ct);
            return;
        ...
        ErrorOr<ObjectStat> ret = await persistentService.GetSegmentAsync(liveId, index, Response.Body, ct);
        if (ret.IsError) {
            Response.StatusCode = StatusCodes.Status404NotFound;
            await Response.WriteAsJsonAsync(ret.Errors, ct);
        }
